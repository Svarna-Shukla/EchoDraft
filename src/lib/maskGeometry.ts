...
// Shared by the light-theme Hero mask and the dark Arena interrogation mask so both use the exact same
// low-poly geometric face rather than diverging implementations.
export function buildMaskGeometries() {
  ...
  // Chin point (bottom of the silhouette) used to anchor the mouth-slit mesh in the arena mask
  const chin = (() => {
    const { z } = profile(0, 0.94);
    return new THREE.Vector3(0, (0.5 - 0.94) * 2.4, z - 0.1);
  })();

  return { shell, backing, eyePositions, chin };
}
